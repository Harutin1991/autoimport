<?php

namespace backend\models;

use yii\base\Model;
use yii\data\ActiveDataProvider;
use backend\models\BrockerAddresses;

/**
 * BrockerAddressesSearch represents the model behind the search form of `backend\models\BrockerAddresses`.
 */
class BrockerAddressesSearch extends BrockerAddresses
{
    /**
     * {@inheritdoc}
     */
    public function rules()
    {
        return [
            [['id', 'brocker_id', 'status', 'product_id'], 'integer'],
            [['address', 'created_at'], 'safe'],
        ];
    }

    /**
     * {@inheritdoc}
     */
    public function scenarios()
    {
        // bypass scenarios() implementation in the parent class
        return Model::scenarios();
    }

    /**
     * Creates data provider instance with search query applied
     *
     * @param array $params
     *
     * @return ActiveDataProvider
     */
    public function search($params,$broker_id = null)
    {
		if(!is_null($broker_id)) {
			$query = BrockerAddresses::find()->where(['brocker_id'=>$broker_id]);
		} else {
			$query = BrockerAddresses::find();
		}
        

        // add conditions that should always apply here

        $dataProvider = new ActiveDataProvider([
            'query' => $query,
        ]);

        $this->load($params);

        if (!$this->validate()) {
            // uncomment the following line if you do not want to return any records when validation fails
            // $query->where('0=1');
            return $dataProvider;
        }

        // grid filtering conditions
        $query->andFilterWhere([
            'id' => $this->id,
            'brocker_id' => $this->brocker_id,
            'status' => $this->status,
            'product_id' => $this->product_id,
            'created_at' => $this->created_at,
        ]);

        $query->andFilterWhere(['like', 'address', $this->address]);

        return $dataProvider;
    }
}
